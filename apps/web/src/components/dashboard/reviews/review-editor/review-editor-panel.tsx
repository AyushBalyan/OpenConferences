...
export function ReviewEditorPanel({ conferenceId, assignmentId }: ReviewEditorPanelProps) {
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Review editor"
        description={`${review.paperTitle ? `${review.paperTitle} · ` : ''}${
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        >
          ...
          {downloadBusy ? 'Preparing…' : canDownload ? 'Download paper' : 'PDF unavailable'}
        </Button>
        {!canDownload ? (
          <p className="text-sm text-muted-foreground">
            The assigned paper PDF is not ready (missing or still scanning).
          </p>
        ) : null}
      </div>
